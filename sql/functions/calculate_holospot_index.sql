...
CREATE OR REPLACE FUNCTION public.calculate_holospot_index(p_user_id uuid)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
DECLARE
    ...
    -- Variáveis para reciprocidade real
...
BEGIN
    ...
    -- =====================================================
    -- 2. RECIPROCIDADE (25%) - MESMA LÓGICA DO IMPACTO DETALHADO
    -- =====================================================
    ...
    LOOP
        ...
        BEGIN
            ...
            -- Participações em correntes que EU criei (conta como 0.5 cada)
            ...
            -- Total B→A (direto + correntes)
            v_total_b_to_a := v_count_b_to_a + v_chain_count;
            
            -- Mínimo entre A→B e B→A
    ...
    -- Calcular índice de reciprocidade
