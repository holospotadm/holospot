...
-- FUNÇÃO 59: handle_feedback_notification_table_debug
-- ============================================================================

CREATE OR REPLACE FUNCTION public.handle_feedback_notification_table_debug()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
DECLARE
    post_author_id UUID;
    username_from TEXT;
    notificacao_criada BOOLEAN := false;
    erro_msg TEXT := NULL;
BEGIN
    -- Salvar início do teste (convertendo UUID para TEXT)
    INSERT INTO public.debug_feedback_test (
        teste_executado, feedback_id, post_id, author_id
    ) VALUES (
        'TRIGGER EXECUTADO', NEW.id::TEXT, NEW.post_id::TEXT, NEW.author_id::TEXT
    );
    ...
    BEGIN
        ...
        INSERT INTO public.debug_feedback_test (
            teste_executado, feedback_id, post_author_id
        ) VALUES (
            'POST AUTHOR ENCONTRADO', NEW.id::TEXT, post_author_id::TEXT
        );
        
    EXCEPTION WHEN OTHERS THEN
        erro_msg := SQLERRM;
        INSERT INTO public.debug_feedback_test (
            teste_executado, feedback_id, erro
        ) VALUES (
            'ERRO AO BUSCAR POST AUTHOR', NEW.id::TEXT, erro_msg
        );
        RETURN NEW;
    END;
    ...
    IF post_author_id IS NULL THEN
        INSERT INTO public.debug_feedback_test (
            teste_executado, feedback_id
        ) VALUES (
            'POST AUTHOR É NULL', NEW.id::TEXT
        );
        RETURN NEW;
    END IF;
    ...
    IF post_author_id = NEW.author_id THEN
        INSERT INTO public.debug_feedback_test (
            ...
            'AUTO-FEEDBACK DETECTADO', NEW.id::TEXT
        );
        RETURN NEW;
    END IF;
    ...
    BEGIN
        ...
        INSERT INTO public.debug_feedback_test (
            ...
            'USERNAME ENCONTRADO', NEW.id::TEXT, username_from
        );
    ...
    EXCEPTION WHEN OTHERS THEN
        ...
        INSERT INTO public.debug_feedback_test (
            ...
            'ERRO AO BUSCAR USERNAME', NEW.id::TEXT, erro_msg
        );
        username_from := 'Usuario';
    END;
    ...
    BEGIN
        ...
        INSERT INTO public.debug_feedback_test (
            ...
            'NOTIFICAÇÃO CRIADA COM SUCESSO', NEW.id::TEXT, true
        );
    ...
    EXCEPTION WHEN OTHERS THEN
        ...
        INSERT INTO public.debug_feedback_test (
            ...
            'ERRO AO CRIAR NOTIFICAÇÃO', NEW.id::TEXT, false, erro_msg
        );
    END;
...
-- FUNÇÃO 117: update_user_total_points
-- ============================================================================
